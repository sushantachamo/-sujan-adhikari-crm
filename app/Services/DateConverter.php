...
class DateConverter
{
     ...
     private $day_num = array('१', '२', '३', '४', '५', '६', '७');
    ...
    public function __construct()
    {
     ...
     public function index()
     {
         ...
         $day_en = date("d",time());
         $date_ne =  $this->get_nepali_date($year_en, $month_en, $day_en);
     ...
     private function get_week_num($year, $month, $day)
     {
         $jd = GregorianToJD($month, $day, $year);
         return $this->day_num[JDDayOfWeek($jd,0)];
     }
     ...
     public function get_nepali_date($year,$month,$day)
     {
         ...
         $daynum = $this->get_week_num($year, $month, $day);
         ...
         for ($i=$this->start_ne; $i<$this->end_ne; $i++)
         {
             ...
             $arr =   $arr +  array_sum($this->nepali_length[$i]);
             ...
             if ($arr>$days)
             {
                 ...
                 for ($j=0; $j < 12; $j++)
                 {
                     ...
                     if($count_previous>$days)
                     {
                         ...
                         $days = ($this->nepali_length[$i][$j]-$daysss)+1;
                         break;
