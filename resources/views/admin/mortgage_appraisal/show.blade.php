    ...
    <section class="rounded mb-3">
        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <a href="{{ url('/admin/mortgage_appraisal') }}" title="Back" type="button" class="btn btn-warning btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a>
                @if($data['row']->deleted_at != null)
                
                @else
                    @can('update-'.Illuminate\Support\Str::lower($panel))
                        <a type="button" href="{{ route($base_route.'.edit', ['mortgage_appraisal'=>$data['row']->id]) }}"
...
<script>
    $(document).ready(function () {
        ...
        $('#pdfExport').on('click', function (e){
            e.preventDefault();
            url = '{{ route('admin.mortgage_appraisalExport.pdf') }}';
        ...
        $('#wordExport').on('click', function (e){
            e.preventDefault();
            url = '{{ route('admin.mortgage_appraisalExport.word') }}';
            $('#export_form').attr('action', url); 
            $('#modalShowExport').modal('hide'); 
            $('#export_form').submit(); 
