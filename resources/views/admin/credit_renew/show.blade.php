    ...
    <section class="rounded mb-3">
        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <a href="{{ url('/admin/credit_renew') }}" title="Back" type="button" class="btn btn-warning btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a>
                @if($data['row']->deleted_at != null)
                
                @else
                    @can('update-'.Illuminate\Support\Str::lower($panel))
                        <a type="button" href="{{ route($base_route.'.edit', ['credit_renew'=>$data['row']->id]) }}"
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="accordion" id="accordionBordered">
                            ...
                            @if($data['row'][config('fields.other_details.approved_by.key')] != null && isset($data['templates']))
                                        ...
                                        <div id="collapse{{$key}}" class="collapse" aria-labelledby="{{$key}}" data-parent="#accordionBordered">
                                            <div class="card-body p-0 ">
                                                ...
                                                <div class="list-group shadow quick-link bg-ebox">
                                                    <a export_id ={{$data['row']->id}} template="{{$template['file_name']}}" class="list-group-item list-group-item-action list-group-item-light exportFile">
...
<script>
    $(document).ready(function () {
        ...
        $('#pdfExport').on('click', function (e){
            e.preventDefault();
            url = '{{ route('admin.credit_renewExport.pdf') }}';
        ...
        $('#wordExport').on('click', function (e){
            e.preventDefault();
            url = '{{ route('admin.credit_renewExport.word') }}';
            $('#export_form').attr('action', url); 
            $('#modalShowExport').modal('hide'); 
            $('#export_form').submit(); 
